...
export const PackageCard: React.FC<PackageCardProps> = ({
  ...
  return (
    <div className={`relative bg-white dark:bg-slate-900/95 rounded-3xl overflow-hidden transition-all duration-300 hover:-translate-y-2 flex flex-col h-full border ${
      pkg.popular 
        ? 'border-party-pink-300 dark:border-party-pink-500/50 shadow-glow ring-2 ring-party-pink-400/20' 
        : 'border-slate-100 dark:border-white/10 shadow-card dark:shadow-card-dark hover:shadow-card-hover dark:hover:shadow-card-hover-dark'
    }`}>
      {/* Popular Badge */}
      {pkg.popular && (
      ...
      {/* Package Image */}
      <div className="relative h-52 overflow-hidden bg-slate-100 dark:bg-slate-800 group">
        <img
          src={pkg.image}
          alt={pkg.name}
        ...
        <div className="absolute bottom-3 left-4 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1.5 shadow-sm border border-white/20 dark:border-white/10">
      ...
      <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
        <div>
          <div className="flex justify-between items-baseline mb-2">
            <h3 className="text-xl font-extrabold text-slate-900 dark:text-white">{pkg.name}</h3>
          ...
          <div className="space-y-2 pt-3 border-t border-slate-100 dark:border-white/10">
            <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Includes:</p>
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          <button
            ...
            className="w-full py-2.5 px-3 rounded-2xl bg-slate-100 dark:bg-slate-800 hover:bg-party-purple-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 hover:text-party-purple-700 dark:hover:text-white text-xs font-bold transition-all border border-slate-200 dark:border-slate-700"
          >
            View Details
          </button>
